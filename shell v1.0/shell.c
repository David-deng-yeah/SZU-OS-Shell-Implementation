...
void conduct_pipe(char buffer[BUFFER_SIZE]){
    ...
    if(idx == -1){// without '|'
        ...
        inputParse(buffer);
        close(pd[1]);// close write port
        dup2(pd[0], STDIN_FILENO);// take pd[0](read port) as std_input
        execvp(argv[0], argv);
        if(pd[0] != STDIN_FILENO){
            close(pd[0]);
        }
        // conduct_cmd(argc, argv);
        return;
    }else if(idx+1 == strlen(buffer)){// '|' without input
        perror("error, | without input\n");
        return;
    }

    /*create inputBuffer and outputBuffer*/
    char outputBuffer[idx];
    memset(outputBuffer, 0x00, idx);
    char inputBuffer[strlen(buffer)-idx];
    memset(inputBuffer, 0x00, strlen(buffer)-idx);
    ...
    for(int i=0; i<strlen(buffer)-idx-1; i++)
        inputBuffer[i] = buffer[idx+2+i];
    // printf("lens: %ld, cet: %s\n",strlen(outputBuffer), outputBuffer);
    // printf("lens: %ld, cet: %s\n",strlen(inputBuffer), inputBuffer);
    
    

    /*recursively use child and parent process to conduct cmd*/
    pid_t pid = fork();
    if(pid == -1){
        perror("error!\n");
    }else if(pid == 0){// child process
        ...
        dup2(pd[1], STDOUT_FILENO);// take pd[1](write port) as std_output
        execvp(argv[0], argv);
        if(pd[1] != STDOUT_FILENO){
            close(pd[1]);
        }
        // conduct_pipe(outputBuffer);
        //printf("%s\n", outputBuffer);
        exit(0);
    }else{// parent process
        int status;
